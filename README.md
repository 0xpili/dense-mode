...
Claude wrote the same 6 responses in each style, measured via word count × 1.3 ([full research](research/dense-vs-cave-talk.md)):
...
The `research/` folder contains the manual empirical comparison:
...
The `benchmark/` folder contains the automated CLI benchmark:

- [run-benchmark.sh](benchmark/run-benchmark.sh) — Reproducible script using `claude -p --output-format json`
- [prompts.json](benchmark/prompts.json) — 6 test prompts
- [results/](benchmark/results/) — Raw JSON output and summaries from all runs
